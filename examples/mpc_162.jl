...
settings_file = "data/2026_162T_NordseeOne.yaml"
vis_file      = "data/vis_162T.yaml"
data_file               = "data/mpc_result_162.jld2"
...
T_EXTRA = 2580+5000   # extra time in addition to sim.end_time for MPC simulation
MIN_INDUCTION = 0.01
...
if (! SIMULATE) && ((isfile(data_file) && !GROUP_CONTROL) || (isfile(data_file_group_control) && GROUP_CONTROL))
    ...
    if GROUP_CONTROL
        if USE_HARDCODED_INITIAL_GUESS
            ...
            elseif GROUPS == 6
                x0 = [1.853, 1.814, 1.507, 1.293, 1.304, 0.08, 1, 0, 2.17, 1.78]
            elseif GROUPS == 2
...
if @isdefined rel_power_ref
    ...
    plot_power_and_demand(time_vector, rel_power, demand_data, rel_power; vis, pltctrl)
end
