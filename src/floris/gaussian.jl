...
yaw range constraints using hyperbolic tangent functions.

# Arguments
- `wf`: Wind farm object containing turbine states, dimensions, and operational data
- `m::Matrix`: Measurement or simulation data matrix where column 3 contains effective wind speeds
- `floris`: FLORIS model parameters containing air density, efficiency factor, and power curve parameters
- `con`: Controller configuration object with yaw control settings and operational constraints

# Returns
- `P::Vector`: Power output for each turbine in the wind farm [W or kW depending on units]

# Description
The function calculates power using the standard wind turbine power equation:
P = 0.5 * ρ * A * Cp * U³ * η * cos(γ)^p_p

Where:
- ρ is air density (floris.airDen)
- A is rotor swept area (π * (D/2)²)
- Cp is power coefficient (4a(1-a)²)
- U is effective wind speed
- η is drivetrain efficiency (floris.eta)
- γ is yaw angle
- `p_p` is yaw power exponent (`floris.p_p`)

# Notes
- Power coefficient is calculated from axial induction factor using Cp = 4a(1-a)²
- Yaw effects are modeled using cos(γ)^p_p term
- Optional yaw range constraints use tanh functions for smooth transitions
- When `con.tanh_yaw` is true, power is constrained within specified yaw range limits
"""
function getPower(wf, m, floris, con)
