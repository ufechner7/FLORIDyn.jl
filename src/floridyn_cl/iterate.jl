...
function iterateOPs!(::IterateOPs_basic, T, Sim, paramFLORIS, paramFLORIDyn)
    ...
    # OPs
    T.States_OP = circshift(T.States_OP, 1; dims=1)
    T.States_OP[T.StartI, :] = tmpOPStates

    # Turbines
    T.States_T = circshift(T.States_T, 1; dims=1)
    T.States_T[T.StartI, :] = tmpTStates

    # Wind Farm
    T.States_WF = circshift(T.States_WF, 1; dims=1)
    T.States_WF[T.StartI, :] = tmpWFSTates

    # Check if OPs are in order
    for iT in 1:T.nT
        inds = T.StartI[iT]:(T.StartI[iT] + T.nOP - 1)
        _, indOP = sort(T.States_OP[inds, 4], dims=1)
        if !issorted(indOP)
            # Sort rows according to the new order
