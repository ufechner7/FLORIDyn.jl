...
# Data Format
The `ConYawData` matrix should have the structure:
```
[time₁  yaw₁₁  yaw₁₂  ...  yaw₁ₙ]
[time₂  yaw₂₁  yaw₂₂  ...  yaw₂ₙ]
[  ⋮      ⋮      ⋮    ⋱     ⋮  ]
...
ConYawData = [0.0  10.0  5.0;   # t=0s: T1=10°, T2=5°
              1.0  15.0  10.0;  # t=1s: T1=15°, T2=10°
              2.0  20.0  15.0]  # t=2s: T1=20°, T2=15°

# Get yaw for turbine 1 at t=0.5s (interpolated)
yaw1 = getYaw(Yaw_SOWFA(), ConYawData, 1, 0.5)  # Returns 12.5°

# Get yaw for multiple turbines at t=1.5s
yaws = getYaw(Yaw_SOWFA(), ConYawData, [1, 2], 1.5)  # Returns [17.5°, 12.5°]

# Out-of-bounds time (will issue warning)
yaw_oob = getYaw(Yaw_SOWFA(), ConYawData, 1, 5.0)  # Returns 20.0° with warning
```
