...
"""
    getMeasurements(mx::Matrix, my::Matrix, nM::Int, zh::Real, wf::WindFarm, set::Settings, 
                    floris::Floris, wind::Wind; threading::Bool=false, alloc=nothing) -> Array{Float64,3}
...
# Keyword Arguments
- `threading::Bool=false`: If true, uses multi-threaded execution with `@threads`
- `alloc=nothing`: Optional allocation tracker for performance monitoring
...
# Performance Notes
- Single-threaded by default, multi-threaded when `threading=true`
- Multi-threaded implementation uses thread-local buffers to avoid race conditions
- Each grid point requires a full wind farm simulation, so computation time scales with grid size
- Scales well with the number of available CPU cores when threading is enabled
...
# Calculate 3 measurements at 90m hub height (single-threaded)
mz = getMeasurements(mx, my, 3, 90.0, wind_farm, settings, floris_model, wind_config)

# Calculate with multi-threading for better performance
mz_parallel = getMeasurements(mx, my, 3, 90.0, wind_farm, settings, floris_model, wind_config; threading=true)
...
"""
function getMeasurements(mx, my, nM, zh, wf::WindFarm, set::Settings, floris::Floris, wind::Wind; threading::Bool=false, alloc=nothing)
    size_mx = size(mx)
...
"""
    getMeasurementsP(mx::Matrix, my::Matrix, nM::Int, zh::Real, wf::WindFarm, set::Settings, 
                     floris::Floris, wind::Wind; alloc=nothing) -> Array{Float64,3}

Parallel version of getMeasurements - maintained for backward compatibility.

This function is a wrapper around `getMeasurements(threading=true)` and is kept
for backward compatibility with existing code.

# Arguments
Same as [`getMeasurements`](@ref)
...
# Note
This function is deprecated. Use `getMeasurements(threading=true)` instead.

# See Also
- [`getMeasurements`](@ref): The unified function that handles both single and multi-threaded execution
"""
function getMeasurementsP(mx, my, nM, zh, wf::WindFarm, set::Settings, floris::Floris, wind::Wind; alloc=nothing)
    return getMeasurements(mx, my, nM, zh, wf, set, floris, wind; threading=true, alloc=alloc)
end
...
function calcFlowField(set::Settings, wf::WindFarm, wind::Wind, floris::Floris; plt=nothing, alloc=nothing)
    ...
    # Get data using unified getMeasurements function
    if set.threading
        ...
        try
            a = @allocated Z = getMeasurements(X, Y, nM, zh, wf, set, floris, wind; threading=true, alloc)
            if ! isnothing(alloc)
        ...
        Z = getMeasurements(X, Y, nM, zh, wf, set, floris, wind; threading=false)
    end
