...
@testset verbose = true "wind dir                                                " begin
    @testset "getWindDirT(Direction_Constant(), ...)" begin
        ...
        WindDir = 270.0
        iT = [1, 2, 3]
        wind = Wind(dir_fixed=WindDir)
        phi = getWindDirT(dir_mode, wind, iT, nothing)
        for ph in phi
            @test ph ≈ 270.0
        end
    end
    ...
    @testset "getWindDirT(Direction_Constant_wErrorCov(), ...)" begin
        ...
        # Test single call
        phi = getWindDirT(dir_mode, wind, iT, 0.0)
        @test length(phi) == 3
        @test all(isfinite.(phi))
        
        # Test statistical properties with multiple samples
        n_samples = 1000
        samples = [getWindDirT(dir_mode, wind, iT, 0.0) for _ in 1:n_samples]
    ...
    @testset "getWindDirT_EnKF(Direction_EnKF_InterpTurbine(), ...)" begin
        ...
        wind.dir = WindDir
        phi = getWindDirT_EnKF(dir_mode, wind, 1, 0.5)
        @test phi ≈ 11.0
    end
    ...
    @testset "getWindDirT(Direction_InterpTurbine(), ...)" begin
        ...
        # Call the function
        phi = getWindDirT(dir_mode, wind, iT, t)
        @test phi ≈ 22.5
    end
    ...
    @testset "getWindDirT(Direction_Interpolation_wErrorCov(), ...)" begin
        ...
        n_samples = 500
        samples = [getWindDirT(dir_mode, wind, iT, t) for _ in 1:n_samples]
    ...
    @testset "getWindDirT(Direction_InterpTurbine_wErrorCov(), ...)" begin
        ...
        # Test single call
        phi = getWindDirT(dir_mode, wind, 1, 12.5)
        @test length(phi) == 1
        @test isfinite(phi[1])
        
        # Test statistical properties with multiple samples
        n_samples = 500
        samples = [getWindDirT(dir_mode, wind, 1, 12.5)[1] for _ in 1:n_samples]
    ...
    @testset "getWindDirT(Direction_RW_with_Mean(), ...)" begin
        ...
        # Test single call - this signature takes wind_dir_now as second parameter
        phi = getWindDirT(dir_mode, WindDirNow, wind)
        @test size(phi) == (3, 1)
        @test all(isfinite.(phi))
        
        # Test statistical properties with multiple samples
        n_samples = 500
        samples = [getWindDirT(dir_mode, WindDirNow, wind) for _ in 1:n_samples]
        sample_matrix = hcat(samples...)  # 3 x n_samples matrix
    ...
    @testset "getWindDirT(Direction_RW_with_Mean(), WindDirTriple, iT, t)" begin
        ...
        @testset "Multiple turbine selection" begin
            ...
            @test size(phi) == (3, 1)  # Matrix format
            @test all(isfinite.(phi))
            
            # Test statistical properties with multiple samples
            n_samples = 800
            samples = [getWindDirT(dir_mode, wind, iT, t) for _ in 1:n_samples]
            sample_matrix = hcat(samples...)  # Combine vectors into matrix
        ...
        @testset "All turbines selection" begin
            ...
            phi = getWindDirT(dir_mode, wind, iT, t)
            @test isa(phi, Matrix{Float64})
            @test size(phi) == (4, 1)  # Matrix format
            @test all(isfinite.(phi))
        ...
        @testset "Edge cases" begin
            ...
            wind_zero = Wind()
            wind_zero.dir = WindDir_zero
            iT = 2
            t = 0.0
            
            phi = getWindDirT(dir_mode, wind_zero, iT, t)
            @test isfinite(phi)
            
            # Test with high mean reversion
            WindDir_high = WindDirTriple(Init, CholSig, 1.0)
            wind_high = Wind()
            wind_high.dir = WindDir_high
            phi_high = getWindDirT(dir_mode, wind_high, iT, t)
            @test isfinite(phi_high)
            
            # Test with single turbine (iT = 1)
            iT_single = 1
            phi_single = getWindDirT(dir_mode, wind, iT_single, t)
        ...
        @testset "Time independence" begin
            ...
            n_samples = 300
            samples_t1 = [getWindDirT(dir_mode, wind, iT, t1) for _ in 1:n_samples]
            samples_t2 = [getWindDirT(dir_mode, wind, iT, t2) for _ in 1:n_samples]
            
            # Convert to regular matrices for statistical analysis
            matrix_t1 = hcat(samples_t1...)
            matrix_t2 = hcat(samples_t2...)
